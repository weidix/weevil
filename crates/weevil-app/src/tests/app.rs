...
#[test]
fn list_script_infos_from_cli_paths() {
    ...
    let infos = script_info::list_script_infos(&config, vec![first, second]).expect("list");
...
#[test]
fn dedupe_script_aliases_with_warning_keeps_earliest_alias() {
    let dir = tempfile::tempdir().expect("temp dir");
    let first = dir.path().join("a.lua");
    let second = dir.path().join("b.lua");
...
#[test]
fn list_script_infos_from_config_paths() {
    let dir = tempfile::tempdir().expect("temp dir");
    ...
    let infos = script_info::list_script_infos(&config, Vec::new()).expect("list");
